...
import os
os.umask(0)  # Sets default permissions to 666 (rw-rw-rw-)
...
with open(file_path, mode='r', encoding='utf-8') as file:
    ...
    for row in reader:
        # Check if one of the source is from the validation set and the other is from the test set
        if row['source_1'] == 'test' and row['source_2'] == 'valid':
            # Create a tuple for the (doc_id_1, doc_id_2) pair
            pair = ( row['doc_id_2'],row['doc_id_1'])
            
            # Increment the count for this pair
            pair_counts[pair] += 1
# Filepath to the output CSV file
#output_file_path = "pair_counts_output_both.csv"
output_file_path = "/Users/xuantongchen/Desktop/pair_counts_output_both.csv"
...
# Load existing pair counts from pair_counts_output_both.csv
#pair_counts_file = "pair_counts_output_both.csv"
pair_counts_file = "/Users/xuantongchen/Desktop/pair_counts_output_both.csv"
...
with open(near_duplicates_file, mode='r', encoding='utf-8') as file:
    ...
    for row in reader:
        # Check if one of the source is from the validation set and the other is from the test set
        if row['set_1'] == 'validation' and row['set_2'] == 'test':
            pair = (row['doc_id_1'], row['doc_id_2'])
            pair_counts[pair] += 1  # Increment count if exists, or set to 1 if new

# Write updated pair_counts back to pair_counts_output_both.csv
...
# Process cleaned_similar_pairs_combined.csv to update pair_counts
...
# Filepath to the pair_counts_output_both.csv file
#pair_counts_file = "pair_counts_output_both.csv"
pair_counts_file = "/Users/xuantongchen/Desktop/pair_counts_output_both.csv"
#output_file = "filtered_pair_counts_both.csv"
output_file = "/Users/xuantongchen/Desktop/filtered_pair_counts_both.csv"

# Initialize a list to store keys with values >= 2
keys_with_high_values = []

# Read the pair_counts_output_both.csv file
with open(pair_counts_file, mode='r', encoding='utf-8') as file:
    reader = csv.DictReader(file)
    for row in reader:
