...
with open(file_path, mode='r', encoding='utf-8') as file:
    ...
    for row in reader:
        # Check if both sources are from the validation set
        if row['source_1'] == 'valid' and row['source_2'] == 'valid':
...
output_file_path = "pair_counts_output_validation.csv"
...
# Load existing pair counts from pair_counts_output_validation.csv
pair_counts_file = "pair_counts_output_validation.csv"
...
with open(near_duplicates_file, mode='r', encoding='utf-8') as file:
    reader = csv.DictReader(file)
    for row in reader:
        # Check if both sources are 'validation'
        if row['set_1'] == 'validation' and row['set_2'] == 'validation':
            pair = (row['doc_id_1'], row['doc_id_2'])
            pair_counts[pair] += 1  # Increment count if exists, or set to 1 if new

# Write updated pair_counts back to pair_counts_output_validation.csv
...
# Process similar_pairs_validation.csv to update pair_counts
similar_pairs_file = "similar_pairs_validation.csv"
...
# Filepath to the pair_counts_output_validation.csv file
pair_counts_file = "pair_counts_output_validation.csv"
output_file = "filtered_pair_counts_validation.csv"

# Initialize a list to store keys with values >= 2
keys_with_high_values = []

# Read the pair_counts_output_validation.csv file
with open(pair_counts_file, mode='r', encoding='utf-8') as file:
    reader = csv.DictReader(file)
    for row in reader:
